export type Locale = 'en' | 'zh'

/** Nested string map — `t('nav.design')` */
export type Messages = typeof enMessages

export const enMessages = {
  lang: {
    switchNav: 'Language',
    enShort: 'EN',
    zhShort: '中',
  },
  nav: {
    primary: 'Primary',
    design: 'Design',
    sideWork: 'Side Work',
    about: 'About Me',
    homeAria: 'li.design — home',
    menuToggle: 'Open site menu',
    menuClose: 'Close site menu',
    externalLinks: 'External links',
  },
  pet: {
    aria: 'Desktop pet Vinny — click to roll, drag to pick up',
    nalaAria: 'Desktop pet Nala — click when annoyed, drag to pick up',
  },
  footer: {
    contactLinks: 'Contact links',
    email: 'Email',
    linkedin: 'LinkedIn',
    resume: 'Resume',
    with: 'with',
    love: 'love',
    vibeCoded: 'Vibe coded 5/2026',
    pet: {
      controls: 'Desktop pets',
      vinnyHi: 'Say Hi to Vinny',
      vinnyBye: 'Say bye to Vinny',
      nalaHi: 'Say Hi to Nala',
      nalaBye: 'Say bye to Nala',
    },
  },
  social: {
    linkedin: 'LinkedIn profile',
    github: 'GitHub profile',
    resumePdf: 'Download resume (PDF)',
  },
  home: {
    selectedWorks: 'Selected Works',
    yearsExperience: 'Years of Experience',
  },
  projects: {
    'mrd-testing-journey': {
      company: 'Tempus AI',
      title: 'Reimagining the MRD\nTesting Journey',
      tag0: 'Healthcare',
      tag1: 'Clinical UX',
      tag2: 'Information Architecture',
      description:
        'Redefining how clinicians track, understand, and act on recurring test progress and results.',
    },
    'tempus-one': {
      company: 'Tempus AI',
      title: 'Tempus One',
      tag0: 'AI Experience',
      tag1: 'Growth Design',
      tag2: '0-1',
      description:
        'How can we leverage AI to help physicians access patient data while grow product usage?',
    },
    'hub-online-ordering': {
      company: 'Tempus AI',
      title: 'Hub Online Ordering',
      tag0: 'Enterprise UX',
      tag1: 'Scalability',
      tag2: 'Healthcare',
      description:
        'How can we utilize the design system to streamline test portfolio expansion for oncologists’ needs?',
    },
    'iqueue-for-clinics': {
      company: 'LeanTaaS',
      title: 'iQueue for Clinics',
      tag0: 'Machine Learning UX',
      tag1: 'Healthcare Ops',
      tag2: 'Data-driven',
      description:
        'How can we leverage predictive analytics to optimize hospital schedule and resource allocation?',
    },
  },
  hero: {
    hi: "Hi, I'm Li! 👋",
    bio: 'A product designer focused on building intuitive tools for complex systems — from generative AI to healthcare platforms.',
    eyebrowPhrases: ['🤖 AI savvy,', '🧑 Human centered,', '💻 Enterprise,', '📊 Data driven'],
    eyebrowAria: 'Rotating focus areas: AI savvy, Human centered, Enterprise, Data driven',
  },
  weather: {
    clear: 'clear skies',
    mainlyClear: 'mainly clear',
    partlyCloudy: 'partly cloudy',
    overcast: 'overcast',
    fog: 'fog',
    rain: 'rain',
    snow: 'snow',
    showers: 'showers',
    snowShowers: 'snow showers',
    thunderstorm: 'thunderstorm',
    generic: 'weather',
    ariaWeather: 'Current weather in Austin, Texas: {temp} degrees Fahrenheit, {desc}',
    ariaClock: 'Local time in Austin, Texas: {time}',
    austin: 'Austin, Texas',
    loading: 'Weather loading…',
    error: 'Weather unavailable',
  },
  notFound: {
    title: 'Page not found',
    description: 'That URL does not exist on this site.',
    back: '← Back to Design',
  },
  caseStudy: {
    moreProjects: 'More projects',
    designDetails: '✍🏻 Design Details',
    designDetailsNav: 'Design detail sections',
    projectTags: 'Project tags',
    intro: 'Intro',
    team: 'Team',
    timeline: 'Timeline',
    projectDetails: 'Project details',
    impact: '🎉 Impact',
    connectCta: 'Curious about the "how"? Let\'s connect! 😊',
    connectCtaAria: 'Get in touch',
  },
  about: {
    photoAlt: {
      me: 'Li Chen',
      vinny: 'Vinny the cat',
      nala: 'Nala the cat',
    },
    photosAria: 'Photos',
    meetHeading: 'nice to meet you!',
    intro:
      'Hi there, this is Li. Besides work, I enjoy traveling ✈️, hunting for cozy coffee shops ☕️, playing tennis 🎾, diving into video games 👾, and playing with my two cats 🐈 🔸Vinny and ▪️Nala.',
    experienceAria:
      'Work and education timeline. Scroll vertically to see earlier roles and education.',
    scrollHint: 'Keep scrolling for more.',
    exp: {
      tempus: {
        dates: '2022 — current',
        title: 'Sr. Product Designer',
        org: 'Tempus AI',
        blurb: 'AI and healthcare product experiences.',
      },
      realChemistry: {
        dates: '2020 — 2022',
        title: 'Designer',
        org: 'Real Chemistry',
        blurb: 'Enterprise healthcare and marketing platforms.',
      },
      leantaas: {
        dates: '2019 — 2020',
        title: 'Product Designer',
        org: 'LeanTaaS',
        blurb: 'Hospital operations and predictive analytics UX.',
      },
      precisionStrategy: {
        dates: '2019',
        title: 'Design intern',
        org: 'Precision Strategy',
        blurb: '',
      },
      parsons: {
        dates: '2017 — 2019',
        title: 'MFA, Design & Technology',
        org: 'Parsons School of Design',
        blurb: 'New York City.',
      },
      hfg: {
        dates: '2015',
        title: 'Visual Communication (exchange)',
        org: 'Hochschule für Gestaltung Offenbach',
        blurb: '',
      },
      nanjing: {
        dates: '2013 — 2016',
        title: 'MA in Graphic Design',
        org: 'Nanjing University of the Arts',
        blurb: '',
      },
      jiangnan: {
        dates: '2009 — 2013',
        title: 'BA, Fine art',
        org: 'Jiangnan University',
        blurb: '',
      },
    },
  },
  sideWork: {
    pageAria: 'Side work',
    dataVizTag: 'Data viz',
    openFullVideo: '{work} — open full video',
    aliceClipsGroup: 'Alice Wonderland clips',
    iplClipsGroup: 'I am Programming Language clips',
    onVimeoIframe: '{title} on Vimeo',
    dataVizOpenTab: 'Open in new tab',
    vimeoCloseBackdrop: 'Close video',
    vimeoClose: 'Close',
    vimeoOpenVimeo: 'Open on Vimeo',
    watchVideo: 'Watch video',
    dataVizTitle: 'The Development of Programming Language',
    dataVizDesc:
      'Interactive data visualization: explore how programming languages spread across nationality, native language, gender, and decade.',
    dataVizIframe:
      'The Development of Programming Language — interactive visualization',
    motionTag: 'Motion graphics',
    vibeCodingTag: 'Vibe Coding',
    typoSandIframe: 'TypoSand — embedded site',
    alice: 'Alice Wonderland',
    ipl: 'I am Programming Language',
    giveMeFish: 'Give Me Fish',
    tomatoUfo: 'Tomato UFO',
    aliceAria: 'Alice Wonderland — open full video',
    iplAria: 'I am Programming Language — open full video',
    giveFishAria: 'Give Me Fish — open full video',
    alicePart: ', part of Alice Wonderland',
    iplPart: ', part of I am Programming Language',
  },
  tempus: {
    articleAria: 'Tempus One case study',
    tags: ['AI Experience', 'Growth Design', '0-1'],
    subline: 'How I turned an underused AI assistant into a growth engine for physicians.',
    intro:
      'Tempus One is a conversational AI assistant integrated into the Hub platform, available on both web and mobile (iOS & Android). It helps external oncologists quickly access patient data and streamline treatment workflows. After the beta launch, I joined as the product design lead to improve user engagement and drive adoption among clinical users.',
    team0: '1 Product Designer',
    team1: '1 Product Manager',
    team2: '1 Operation Manager',
    team3: '1 Marketing Manager',
    team4: '5 Engineers',
    timelineRange: 'May 2023 - October 2024',
    problem: 'Product problem',
    problemDetail:
      'Beta launch brings the user stickiness issue with low acquisition and retention rate, however, business goal is to grow user base.',
    problemDataHeading: 'Data shows:',
    problemPoint1Before: 'Only ',
    problemPoint1Highlight: '54%',
    problemPoint1After: ' of advocates tried the product',
    problemPoint2: 'Very few sessions per user',
    problemPoint3: 'Usage decreased after provisioning',
    problemPoint4Before: 'Response success rate was already ',
    problemPoint4Highlight: '72.55%',
    problemConclusion: "The problem wasn't answer quality. It was product adoption.",
    timelineHeading: 'Timeline',
    impactHeading: 'Impact',
    impactStats: {
      unique: { value: '+685.7%', label: 'Unique users' },
      mau: { value: '+48.8%', label: 'MAU' },
      retention: { value: '19.4% → 34.7%', label: 'Retention' },
    },
    impactSummary:
      'Design helped turn Tempus One from a promising beta product into a growing part of the Hub experience.',
    customerAssistant: 'Evolving the Assistant with GenAI',
    customerAssistantIntro:
      'With the introduction of GenAI, Customer Assistant evolved from a rule-based experience into an AI-powered agent, expanding the range of physician inquiries it could handle.',
    successHeading: 'Success since launch',
    caStat1: 'of user inquiry are answered by Customer Assistant Agent',
    caStat2: 'success rate -',
    caStat2b: 'Agent success rate improvement',
    figmaTitle: 'Figma prototype: CAA Demo',
    orderingLaunch: 'From Answering to Acting',
    orderingIntro:
      'The next opportunity was to move beyond answering questions and help physicians complete tasks directly within the conversation.',
    orderingStat1Before: 'Highest adoption:',
    orderingStat1After: 'usage vs next feature',
    orderingStat2Before: 'Current usage rank',
    orderingFeatures: {
      orderTest: { label: 'Order test', alt: 'Order test in Tempus One conversation' },
      orderAddOn: { label: 'Order add-on', alt: 'Order add-on in Tempus One conversation' },
      orderSampleKit: { label: 'Order sample kit', alt: 'Order sample kit in Tempus One conversation' },
      cancelOrder: { label: 'Cancel order', alt: 'Cancel order in Tempus One conversation' },
    },
    timeline: {
      beta: { title: 'Beta Launch', date: '05.2023' },
      analysis: { title: 'User Analysis & Product Strategy' },
      fte: { title: 'First Time Experience Redesign (Activation)' },
      national: { title: 'National Launch', date: '09.2023' },
      ca: { title: 'Customer Assistant Launch (1st Gen AI agent)' },
      ordering: { title: 'Ordering Capability Launch' },
      lit: { title: 'Literature Search Launch' },
      ipo: { title: 'Company IPO', date: '06.2024' },
      rebrand: { title: 'Product Rebrand' },
    },
  },
  hub: {
    articleAria: 'Hub Online Ordering case study',
    tags: ['Enterprise UX', 'Scalability', 'Healthcare'],
    subline:
      'Redesigned and scaled online ordering experience, increasing order volume from 23K to 40K.',
    intro:
      'Oncology NGS testing is central to Tempus, but the ordering experience was complex and frustrating due to varying test requirements. As the business grew, adding new tests made the order form harder to maintain, challenging the goal of increasing order volume. I helped redesign a scalable order form and integrate the new xG Test.',
    team0: '2 Product Designers',
    team1: '1 Product Manager',
    team2: '6 Engineers',
    timelineRange: 'Sept 2022 - April 2023',
    slides: {
      testSelection: { title: 'Test selection', alt: 'Test selection step in Hub online ordering' },
      autofill: {
        title: 'Existing patient auto fill',
        alt: 'Existing patient auto fill in Hub online ordering',
      },
      confirm: { title: 'Confirmation page', alt: 'Order confirmation page in Hub online ordering' },
    },
    impact: {
      fail: 'failure rate on order submission',
      timeBefore: 'Reduce submission time from',
      timePct: '40%',
      timeMid: '(',
      timeFrom: '~9 seconds',
      timeTo: 'to',
      timeFast: '<3 seconds',
      timeAfter: ') post national rollout',
      volBefore: 'Increase online order form volume from',
      volFrom: '27k',
      volTo: '40k',
      volAfter: 'YTD in 2023',
    },
  },
  iqueue: {
    articleAria: 'iQueue for Clinics case study',
    tags: ['Machine Learning UX', 'Healthcare', 'Data-driven'],
    subline:
      'How can we leverage predict analytic to optimize hospital schedule and resource allocation?',
    intro:
      'iQueue for Clinics is a web-based analytics tool helping healthcare professionals to reduce patient wait time and maximize resource allocation. Room Allocation is one portion of the Clinic Admin Panel that helps administrators optimize utilization of exam and procedure rooms.',
    team0: '1 Product Designer',
    team1: '1 Product Manager',
    team2: '1 Data Scientist',
    team3: '2 Product Implementation Managers',
    team4: '14 Engineers',
    timelineRange: 'Dec 2019 - March 2020',
    slides: {
      dataHealth: { title: 'Data Health', alt: 'Data Health in iQueue for Clinics' },
      room: { title: 'Room allocation', alt: 'Room allocation view in iQueue for Clinics' },
      provider: {
        title: 'Provider Template Review',
        alt: 'Provider Template Review in iQueue for Clinics',
      },
    },
    takeawaysHeading: '📒 Key Takeaways',
    takeaway0: 'What User require not always means what they need',
    takeaway1: 'Involving engineering team from the outset can save time in an agile environment',
    takeaway2: "Try to find resources when there aren't enough, be proactive as the only designer on the team",
  },
  mrd: {
    articleAria: 'Reimagining the MRD Testing Journey case study',
    tags: ['Healthcare', 'Clinical UX', 'Information Architecture'],
    subline:
      'Redefining how clinicians track, understand, and act on recurring test progress and results.',
    intro:
      "The launch of a new monitoring test introduced a fundamentally different testing workflow. Instead of a single test, clinicians now needed to manage and monitor a series of tests over time, making visibility into each test's status critical. Our existing tracking experience wasn't designed for this new mental model.",
    team0: '1 Product Designer',
    team1: '1 Product Manager',
    team2: '2 Operation Managers',
    team3: '1 Commercial Strategist',
    team4: '4 Engineers',
    timelineRange: 'September 2025 - April 2026',
    theProblem: 'The problem',
    theProblemText:
      'Our product was built around a single-test mental model. Monitoring introduced a fundamentally different one.',
    theProblemSingleTest: 'Single test: Test → Status → Result',
    theProblemSingleTestImageAlt: 'Single test workflow: test, status, and result',
    theProblemMonitoring:
      'Monitoring: Series → Test 1 → Status → Result → Test 2 → Status → Result → Test 3 → Status → Result',
    theProblemMonitoringImageAlt: 'Monitoring workflow across a series of tests with status and results',
    problem: 'My role',
    problemLead:
      'As the design lead, I identified the gap and redefined the tracking experience to help clinicians understand the progress and status of an entire monitoring journey.',
    problemDetail:
      'During the project, I uncovered the same underlying information architecture problem in the results experience. Rather than solving them independently, I developed a unified design approach that created a more cohesive experience across both tracking and results.',
    reframingProblem: 'Reframing the problem',
    reframingProblemText:
      'I started exploring a new navigation model to address the core challenge: how users track and return to monitoring series. What started as a tracking problem became a product-wide structure problem.',
    reframingTestTracking: 'Test tracking',
    reframingTestTrackingSub: 'Single test vs. Recurring series',
    reframingTestTrackingImageAlt: 'Test tracking experience redesign',
    reframingTestResult: 'Test result',
    reframingTestResultSub: 'Individual result vs. Results across a monitoring journey',
    reframingTestResultImageAlt: 'Test result experience redesign',
    reframingUnderlyingProblemLabel: 'Underlying problem:',
    reframingUnderlyingProblemText:
      'The product had no shared structure for understanding a series of tests over time.',
    reframingProblemClosing:
      'Product and Engineering supported expanding the scope because both tracking and resulting experiences were manifestations of the same underlying problem.',
    designingSystem: 'Designing the system',
    designingResultTypes: 'Different result types',
    designingResultTypesImageAlt: 'Different result types interaction',
    designingVariableCanvas: 'Variable canvas',
    designingVariableCanvasImageAlt: 'Variable canvas interaction',
    outcome: 'Outcome',
    outcomeText:
      'Launched in Q1 2026. Established a unified experience for monitoring test tracking and results, creating a scalable model for supporting the new longitudinal testing workflow.',
  },
} as const

export const zhMessages = {
  lang: {
    switchNav: '语言',
    enShort: 'EN',
    zhShort: '中',
  },
  nav: {
    primary: '主导航',
    design: '设计作品',
    sideWork: '副业创作',
    about: '关于我',
    homeAria: 'li.design — 首页',
    menuToggle: '打开网站菜单',
    menuClose: '关闭网站菜单',
    externalLinks: '外部链接',
  },
  pet: {
    aria: '桌面电子宠物 Vinny — 点击翻滚，拖动拎起',
    nalaAria: '桌面电子宠物 Nala — 点击表示烦躁，拖动拎起',
  },
  footer: {
    contactLinks: '联系方式',
    email: '邮箱',
    linkedin: '领英',
    resume: '简历',
    with: '使用',
    love: '喜爱',
    vibeCoded: '2026 年 5 月 Vibe coding',
    pet: {
      controls: '桌面宠物',
      vinnyHi: '和 Vinny 打个招呼',
      vinnyBye: '和 Vinny 说再见',
      nalaHi: '和 Nala 打个招呼',
      nalaBye: '和 Nala 说再见',
    },
  },
  social: {
    linkedin: '领英个人主页',
    github: 'GitHub 主页',
    resumePdf: '下载简历（PDF）',
  },
  home: {
    selectedWorks: '精选作品',
    yearsExperience: '从业年数',
  },
  projects: {
    'mrd-testing-journey': {
      company: 'Tempus AI',
      title: 'Reimagining the MRD\nTesting Journey',
      tag0: '医疗',
      tag1: '临床体验',
      tag2: '信息架构',
      description: '重新定义临床医生如何追踪、理解周期性检测进展与结果，并据此采取行动。',
    },
    'tempus-one': {
      company: 'Tempus AI',
      title: 'Tempus One',
      tag0: 'AI 体验',
      tag1: '增长设计',
      tag2: '0-1',
      description:
        '如何利用 AI 帮助医生获取患者数据，并提升产品使用？',
    },
    'hub-online-ordering': {
      company: 'Tempus AI',
      title: 'Hub 在线订购',
      tag0: '企业级体验',
      tag1: '可扩展性',
      tag2: '医疗',
      description:
        '如何利用设计系统简化检测项目扩展，以满足肿瘤医生的需求？',
    },
    'iqueue-for-clinics': {
      company: 'LeanTaaS',
      title: 'iQueue for Clinics',
      tag0: '机器学习体验',
      tag1: '医疗运营',
      tag2: '数据驱动',
      description:
        '如何利用预测分析优化医院排程与资源配置？',
    },
  },
  hero: {
    hi: '你好，我是 Li！👋',
    bio: '我是一名产品设计师，致力于在复杂系统中打造易用的工具——从生成式 AI 到医疗健康平台。',
    eyebrowPhrases: ['🤖 懂 AI，', '🧑 以人为本，', '💻 企业级体验，', '📊 数据驱动'],
    eyebrowAria: '轮播关键词：懂 AI、以人为本、企业级、数据驱动',
  },
  weather: {
    clear: '晴朗',
    mainlyClear: '大部晴朗',
    partlyCloudy: '多云',
    overcast: '阴天',
    fog: '雾',
    rain: '雨',
    snow: '雪',
    showers: '阵雨',
    snowShowers: '阵雪',
    thunderstorm: '雷暴',
    generic: '天气',
    ariaWeather: '德克萨斯州奥斯汀当前天气：{temp} 华氏度，{desc}',
    ariaClock: '德克萨斯州奥斯汀当地时间：{time}',
    austin: '德克萨斯州奥斯汀',
    loading: '天气加载中…',
    error: '天气暂不可用',
  },
  notFound: {
    title: '页面未找到',
    description: '本站没有该链接对应的页面。',
    back: '← 返回设计首页',
  },
  caseStudy: {
    moreProjects: '更多项目',
    designDetails: '✍🏻 设计细节',
    designDetailsNav: '设计细节小节',
    projectTags: '项目标签',
    intro: '概述',
    team: '团队',
    timeline: '时间线',
    projectDetails: '项目信息',
    impact: '🎉 成果',
    connectCta: '好奇「怎么做」？欢迎联系我！😊',
    connectCtaAria: '联系作者',
  },
  about: {
    photoAlt: {
      me: '陈 Li',
      vinny: '猫 Vinny',
      nala: '猫 Nala',
    },
    photosAria: '照片',
    meetHeading: '很高兴认识你！',
    intro:
      '你好，我是 Li。工作之余我喜欢旅行 ✈️、寻找温馨的咖啡馆 ☕️、打网球 🎾、玩电子游戏 👾，还有陪两只猫 🐈 🔸Vinny 和 ▪️Nala 玩。',
    experienceAria: '工作与教育经历时间线。上下滚动可查看更早的职位与学业。',
    scrollHint: '继续滚动查看更多。',
    exp: {
      tempus: {
        dates: '2022 — 至今',
        title: '高级产品设计师',
        org: 'Tempus AI',
        blurb: 'AI 与医疗健康产品体验。',
      },
      realChemistry: {
        dates: '2020 — 2022',
        title: '设计师',
        org: 'Real Chemistry',
        blurb: '医疗健康与营销类企业级平台。',
      },
      leantaas: {
        dates: '2019 — 2020',
        title: '产品设计师',
        org: 'LeanTaaS',
        blurb: '医院运营与预测分析相关体验设计。',
      },
      precisionStrategy: {
        dates: '2019',
        title: '设计实习生',
        org: 'Precision Strategy',
        blurb: '',
      },
      parsons: {
        dates: '2017 — 2019',
        title: '设计科技 MFA',
        org: 'Parsons School of Design',
        blurb: '纽约市。',
      },
      hfg: {
        dates: '2015',
        title: '视觉传达（交换）',
        org: 'Hochschule für Gestaltung Offenbach',
        blurb: '',
      },
      nanjing: {
        dates: '2013 — 2016',
        title: '平面设计硕士',
        org: '南京艺术学院',
        blurb: '',
      },
      jiangnan: {
        dates: '2009 — 2013',
        title: '美术学学士',
        org: '江南大学',
        blurb: '',
      },
    },
  },
  sideWork: {
    pageAria: '副业创作',
    dataVizTag: '数据可视化',
    openFullVideo: '{work} — 打开完整视频',
    aliceClipsGroup: 'Alice Wonderland 片段',
    iplClipsGroup: 'I am Programming Language 片段',
    onVimeoIframe: '{title}（Vimeo）',
    dataVizOpenTab: '在新标签页打开',
    vimeoCloseBackdrop: '关闭视频',
    vimeoClose: '关闭',
    vimeoOpenVimeo: '在 Vimeo 打开',
    watchVideo: '观看视频',
    dataVizTitle: '编程语言的发展',
    dataVizDesc:
      '交互式数据可视化：探索编程语言如何随国籍、母语、性别与年代传播。',
    dataVizIframe: '《编程语言的发展》交互式可视化',
    motionTag: '动效与图形',
    vibeCodingTag: 'Vibe 编程',
    typoSandIframe: 'TypoSand — 嵌入站点',
    alice: 'Alice Wonderland',
    ipl: 'I am Programming Language',
    giveMeFish: 'Give Me Fish',
    tomatoUfo: '番茄 UFO',
    aliceAria: 'Alice Wonderland — 打开完整视频',
    iplAria: 'I am Programming Language — 打开完整视频',
    giveFishAria: 'Give Me Fish — 打开完整视频',
    alicePart: '，Alice Wonderland 片段',
    iplPart: '，I am Programming Language 片段',
  },
  tempus: {
    articleAria: 'Tempus One 案例研究',
    tags: ['AI 体验', '增长设计', '0-1'],
    subline: '我如何将一款使用率偏低的 AI 助手，打造成面向医生的增长引擎。',
    intro:
      'Tempus One 是集成在 Hub 平台的对话式 AI 助手，支持 Web 与移动端（iOS 与 Android）。它帮助外部肿瘤医生快速获取患者数据并简化治疗流程。Beta 发布后，我作为产品设计负责人加入，以提升用户参与度并推动临床采用。',
    team0: '1 名产品设计师',
    team1: '1 名产品经理',
    team2: '1 名运营经理',
    team3: '1 名市场经理',
    team4: '5 名工程师',
    timelineRange: '2023 年 5 月 — 2024 年 10 月',
    problem: '产品问题',
    problemDetail:
      'Beta 阶段存在用户粘性不足、获客与留存偏低的问题，而业务目标仍是扩大用户规模。',
    problemDataHeading: '数据显示：',
    problemPoint1Before: '仅 ',
    problemPoint1Highlight: '54%',
    problemPoint1After: ' 的 advocate 尝试过产品',
    problemPoint2: '每位用户的会话次数很少',
    problemPoint3: '开通后使用量下降',
    problemPoint4Before: '响应成功率已达 ',
    problemPoint4Highlight: '72.55%',
    problemConclusion: '问题不在于回答质量，而在于产品采用。',
    timelineHeading: '时间线',
    impactHeading: '成果',
    impactStats: {
      unique: { value: '+685.7%', label: '独立用户' },
      mau: { value: '+48.8%', label: '月活跃用户' },
      retention: { value: '19.4% → 34.7%', label: '留存率' },
    },
    impactSummary:
      '设计帮助 Tempus One 从有潜力的 Beta 产品，成长为 Hub 体验中持续扩张的一环。',
    customerAssistant: '用 GenAI 演进助手体验',
    customerAssistantIntro:
      '随着 GenAI 的引入，Customer Assistant 从基于规则的体验演进为 AI 驱动的智能体，能够处理的医生咨询范围也随之扩大。',
    successHeading: '上线以来成效',
    caStat1: '的用户咨询由 Customer Assistant 智能体解答',
    caStat2: '成功率 —',
    caStat2b: '智能体成功率提升',
    figmaTitle: 'Figma 原型：CAA Demo',
    orderingLaunch: '从回答到行动',
    orderingIntro:
      '下一步机会是超越回答问题，帮助医生在对话中直接完成任务。',
    orderingStat1Before: '最高采用率：',
    orderingStat1After: '于下一功能的使用量',
    orderingStat2Before: '当前使用量排名',
    orderingFeatures: {
      orderTest: { label: '订购检测', alt: 'Tempus One 对话中的订购检测' },
      orderAddOn: { label: '订购附加项', alt: 'Tempus One 对话中的订购附加项' },
      orderSampleKit: { label: '订购采样 kit', alt: 'Tempus One 对话中的订购采样 kit' },
      cancelOrder: { label: '取消订单', alt: 'Tempus One 对话中的取消订单' },
    },
    timeline: {
      beta: { title: 'Beta 上线', date: '2023.05' },
      analysis: { title: '用户分析与产品策略' },
      fte: { title: '首次体验重设计（激活）' },
      national: { title: '全国上线', date: '2023.09' },
      ca: { title: 'Customer Assistant 上线（第一代 AI 智能体）' },
      ordering: { title: '订购能力上线' },
      lit: { title: '文献检索上线' },
      ipo: { title: '公司 IPO', date: '2024.06' },
      rebrand: { title: '产品品牌焕新' },
    },
  },
  hub: {
    articleAria: 'Hub 在线订购案例研究',
    tags: ['企业级体验', '可扩展性', '医疗'],
    subline: '重设计并扩展在线订购体验，将订单量从 23K 提升至 40K。',
    intro:
      '肿瘤 NGS 检测是 Tempus 的核心业务，但订购流程因检测要求不一而复杂难用。随着业务扩张，新增检测使表单更难维护，也阻碍提升订单量的目标。我参与重设计可扩展的订购表单并整合新的 xG Test。',
    team0: '2 名产品设计师',
    team1: '1 名产品经理',
    team2: '6 名工程师',
    timelineRange: '2022 年 9 月 — 2023 年 4 月',
    slides: {
      testSelection: { title: '检测选择', alt: 'Hub 在线订购中的检测选择步骤' },
      autofill: { title: '已有患者自动填写', alt: 'Hub 在线订购中的已有患者自动填写' },
      confirm: { title: '确认页', alt: 'Hub 在线订购的订单确认页' },
    },
    impact: {
      fail: '的订单提交失败率',
      timeBefore: '全国推广后将提交时间缩短',
      timePct: '40%',
      timeMid: '（从约',
      timeFrom: '9 秒',
      timeTo: '到',
      timeFast: '小于 3 秒',
      timeAfter: '）',
      volBefore: '在线订购表单量从',
      volFrom: '27k',
      volTo: '提升至 40k',
      volAfter: '（2023 年累计）',
    },
  },
  iqueue: {
    articleAria: 'iQueue for Clinics 案例研究',
    tags: ['机器学习体验', '医疗', '数据驱动'],
    subline: '如何利用预测分析优化医院排程与资源配置？',
    intro:
      'iQueue for Clinics 是一款帮助医疗专业人员缩短患者等候时间、优化资源配置的 Web 分析工具。诊室分配是诊所管理后台的一部分，帮助管理员优化检查室与手术室等资源利用率。',
    team0: '1 名产品设计师',
    team1: '1 名产品经理',
    team2: '1 名数据科学家',
    team3: '2 名产品实施经理',
    team4: '14 名工程师',
    timelineRange: '2019 年 12 月 — 2020 年 3 月',
    slides: {
      dataHealth: { title: '数据健康', alt: 'iQueue for Clinics 中的数据健康' },
      room: { title: '诊室分配', alt: 'iQueue for Clinics 中的诊室分配视图' },
      provider: { title: '医师模板审核', alt: 'iQueue for Clinics 中的医师模板审核' },
    },
    takeawaysHeading: '📒 关键收获',
    takeaway0: '用户说的「想要」并不总是他们真正需要的',
    takeaway1: '在敏捷环境中尽早拉工程参与可以节省时间',
    takeaway2: '作为唯一设计师时要主动寻找资源、积极补位',
  },
  mrd: {
    articleAria: '重新构想 MRD 检测之旅案例研究',
    tags: ['医疗', '临床体验', '信息架构'],
    subline: '重新定义临床医生如何追踪、理解周期性检测进展与结果，并据此采取行动。',
    intro:
      '新监测检测的推出带来了根本不同的检测工作流。临床医生不再只需处理单次检测，而需要在时间维度上管理与监测一系列检测，因此看清每项检测的状态变得至关重要。我们原有的追踪体验并未围绕这一新的心智模型设计。',
    team0: '1 名产品设计师',
    team1: '1 名产品经理',
    team2: '2 名运营经理',
    team3: '1 名商业策略师',
    team4: '4 名工程师',
    timelineRange: '2025 年 9 月 — 2026 年 4 月',
    theProblem: '问题',
    theProblemText:
      '我们的产品建立在单次检测的心智模型之上，而监测引入了一种根本不同的心智模型。',
    theProblemSingleTest: '单次检测：检测 → 状态 → 结果',
    theProblemSingleTestImageAlt: '单次检测工作流：检测、状态与结果',
    theProblemMonitoring:
      '监测：系列 → 检测 1 → 状态 → 结果 → 检测 2 → 状态 → 结果 → 检测 3 → 状态 → 结果',
    theProblemMonitoringImageAlt: '监测系列中多项检测的状态与结果工作流',
    problem: '我的角色',
    problemLead:
      '作为设计负责人，我发现该缺口并重定义了追踪体验，帮助临床医生理解整条监测旅程的进展与状态。',
    problemDetail:
      '项目过程中，我在结果体验中发现了同样的底层信息架构问题。我没有分别解决，而是制定了统一的设计方案，在追踪与结果之间打造更连贯的体验。',
    reframingProblem: '重新框定问题',
    reframingProblemText:
      '我开始探索新的导航模型，以应对核心挑战：用户如何追踪并回到监测系列。最初是追踪问题，后来演变为整个产品结构问题。',
    reframingTestTracking: '检测追踪',
    reframingTestTrackingSub: '单次检测 vs. 周期性系列',
    reframingTestTrackingImageAlt: '检测追踪体验重设计',
    reframingTestResult: '检测结果',
    reframingTestResultSub: '单项结果 vs. 监测旅程中的多项结果',
    reframingTestResultImageAlt: '检测结果体验重设计',
    reframingUnderlyingProblemLabel: '底层问题：',
    reframingUnderlyingProblemText: '产品缺乏用于理解随时间推移的检测系列的共享结构。',
    reframingProblemClosing:
      '产品与工程团队支持扩大范围，因为追踪与结果两种体验本质上是同一底层问题的不同呈现。',
    designingSystem: '系统设计',
    designingResultTypes: '不同结果类型',
    designingResultTypesImageAlt: '不同结果类型交互演示',
    designingVariableCanvas: '可变画布',
    designingVariableCanvasImageAlt: '可变画布交互演示',
    outcome: '成果',
    outcomeText:
      '于 2026 年第一季度上线。建立了监测检测追踪与结果的统一体验，为支持新的纵向检测工作流打造了可扩展模式。',
  },
} as unknown as Messages
